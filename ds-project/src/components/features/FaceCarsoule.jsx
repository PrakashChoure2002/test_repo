...
>
  ...
  />
      <img src={item.image[0].imagemod1} alt={item.name} className="max-h-40 w-auto"/>

  <div className="absolute inset-0 bg-black opacity-10 group-hover:opacity-50"/>
  <div className="relative flex flex-col gap-3">
    <item.icon className="text-blue-600 group-hover:text-blue-400 w-[32px] h-[32px]"/>
    <h1 className="text-lg lg:text-xl xl:text-2xl">{item.name}</h1>
    <p className="text-sm lg:text-base xl:text-lg">{item.price}</p>
    <p className="text-sm lg:text-base xl:text-lg">{item.category}</p>
    <p className="text-sm lg:text-base xl:text-lg">{item.description}</p>
